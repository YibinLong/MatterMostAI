...
# Insert test data directly into Mattermost PostgreSQL database
# Creates 100 messages + 50 thread replies per channel

# Channel IDs
MAJOR_PROJECT="7iesgmojn7gt3yigux1ubwpnqh"
WAR_ROOM="wukg1zn9hpgymmba65ocai58zc"
CRITICAL_BUG="ki6rxazzqbrd3d96yfjjwxte1h"

# User IDs
ALICE="1erhepqa6idtbnkkxhxaji8kjr"
BOB="6nccrpw7sjdw3yzn38buhhigrw"
CHARLIE="9fbpoq8e6pypdejbz9dg5dyffe"
...
# Generate SQL
generate_sql() {
    echo "-- Delete existing test messages first"
    echo "DELETE FROM posts WHERE channelid IN ('$MAJOR_PROJECT', '$WAR_ROOM', '$CRITICAL_BUG') AND type = '';"
    echo ""

    USERS=("$ALICE" "$BOB" "$CHARLIE")

    # Time ranges (in milliseconds)
    # NOW minus various periods
    HOUR_AGO=$((NOW - 3600000))
    DAY_AGO=$((NOW - 86400000))
    WEEK_AGO=$((NOW - 604800000))
    MONTH_AGO=$((NOW - 2592000000))

    for channel_name in "major-project" "war-room" "critical-bug"; do
        if [ "$channel_name" = "major-project" ]; then
            CHANNEL_ID="$MAJOR_PROJECT"
...
echo "$SQL" | docker exec -i -e PGPASSWORD=mostest mattermost-postgres psql -U mmuser -d mattermost_test
